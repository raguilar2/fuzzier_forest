# id goes from 1 to 800 for simulations run on a cluster of workstations.
...
for (l in 1:sim_number) {
  ...
  cif <- cforest(y ~ ., data = df, controls = cforest_control(mtry = floor(p * 
    mtry_factor), ntree = ntree))
  vim <- varimp(cif, conditional = TRUE)
  vim <- as.data.frame(cbind(names(vim), vim))
